...
function switchView(name){$$('.view').forEach(v=>v.classList.toggle('active',v.id===name+'View'));$$('[data-view]').forEach(x=>x.classList.toggle('active',x.dataset.view===name));if(name==='connections')renderConnections();if(name==='indicators')renderIndicatorDashboard();if(name==='customindex'){renderCixTokenOptions();renderCixTemplateOptions();updateCixFormulaPreview();renderCixLibrary()}window.scrollTo({top:0,behavior:'smooth'})}
