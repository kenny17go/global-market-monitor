...
let DATA=null;
let alerts=(()=>{try{const v=JSON.parse(localStorage.getItem('gmmAlerts')||'[]');return Array.isArray(v)?v:[]}catch{return []}})();
let selectedTop=(()=>{try{const v=JSON.parse(localStorage.getItem('gmmTopCards')||'null');return Array.isArray(v)?v:['SPX','SOX','TAIEX','NIKKEI','GOLD','WTI','USDTWD']}catch{return ['SPX','SOX','TAIEX','NIKKEI','GOLD','WTI','USDTWD']}})();
...
const DISPLAY_DECIMALS={TX:0,MTX:0,SPF:2,MES:2,ES:2,UNF:0,MNQ:2,NQ:2,UDF:0,MYM:0,YM:0,SXF:1,SOX:1,TJF:1,'mini-TOPIX':2,TOPIX:1,F1F:1,Z:1,RHF:4,CNH:4,XEF:4,'6E':4,XJF:2,'6J':6,XBF:4,'6B':4,XAF:4,'6A':4,GDF:1,MGC:1,GC:1,TGF:0,BRF:2,IMM:3,BRENT:2};

const INITIAL_MARGINS={
...
const CONTRACT_SPECS={
  TAIFEX_TX:{code:'TX',multiplier:200,tick:1,currency:'TWD',cycle:'monthly6',label:'臺指期',note:'TWD 200 / index point'},
  TAIFEX_MTX:{code:'MTX',multiplier:50,tick:1,currency:'TWD',cycle:'monthly6',label:'小型臺指 MTX',note:'TWD 50 / index point'},
  ...
  TAIFEX_TJF:{code:'TJF',multiplier:200,tick:0.25,currency:'TWD',cycle:'spot2q3',label:'TOPIX',note:'TWD 200 / index point; tick TWD 50'},
  ...
  TAIFEX_XBF:{code:'XBF',multiplier:20000,tick:0.0001,currency:'USD',cycle:'quarter4',label:'GBP/USD',note:'GBP 20,000; tick USD 2'},
  ...
  TAIFEX_TGF:{code:'TGF',multiplier:10,tick:0.5,currency:'TWD',cycle:'even6',label:'Gold TWD',note:'1 tael (10 mace / 37.5 g); quoted per mace; tick TWD 5'},
...
function renderCustomIndicators(){
  ...
  el.innerHTML=customIndicators.length?customIndicators.map(ind=>{const v=indicatorValue(ind);return '<div class="indicator-card"><div><b>'+ind.name+'</b><small>'+ind.formula+'</small></div><div class="indicator-card-value">'+(v==null?'—':tidy(v,8))+'</div><button class="secondary-btn" data-edit-indicator="'+ind.id+'">編輯</button><button class="secondary-btn" data-del-indicator="'+ind.id+'">刪除</button></div>'}).join(''):'<div class="source-note">尚未建立新指標。先在上方完成公式，再命名儲存。</div>';
  $$('[data-edit-indicator]').forEach(b=>b.onclick=()=>{const ind=customIndicators.find(x=>x.id===b.dataset.editIndicator);if(!ind||!$('#spreadFormula'))return;editingIndicatorId=ind.id;$('#spreadFormula').value=ind.formula;if($('#indicatorName'))$('#indicatorName').value=ind.name;const save=$('#saveIndicator');if(save)save.textContent='更新指標';calcFormula();$('#indicatorName')?.focus()});
  $$('[data-del-indicator]').forEach(b=>b.onclick=()=>{const id=b.dataset.delIndicator;if(editingIndicatorId===id){editingIndicatorId=null;const save=$('#saveIndicator');if(save)save.textContent='儲存新指標'}customIndicators=customIndicators.filter(x=>x.id!==id);indicatorAlerts=indicatorAlerts.filter(x=>x.indicatorId!==id);saveIndicators();saveIndicatorAlerts();renderCustomIndicators();renderIndicatorAlertControls()});
}
...
function bindIndicatorUI(){
  ...
  if($('#cancelIndicatorEdit'))$('#cancelIndicatorEdit').onclick=()=>{editingIndicatorId=null;if($('#indicatorName'))$('#indicatorName').value='';if($('#spreadFormula'))$('#spreadFormula').value='';if($('#formulaResult')){$('#formulaResult').className='formula-result';$('#formulaResult').textContent='等待輸入公式'}const save=$('#saveIndicator');if(save)save.textContent='儲存新指標'};
  if($('#spreadFormula'))$('#spreadFormula').addEventListener('input',()=>{if($('#formulaResult'))calcFormula()});
  if($('#saveIndicator'))$('#saveIndicator').onclick=()=>{const name=$('#indicatorName')?.value.trim(),formula=$('#spreadFormula')?.value.trim();if(!name)return alert('請先輸入新指標名稱');if(!formula)return alert('請先建立自設公式');const r=evalMarketFormula(formula);if(!r.ok||r.type!=='number'||!Number.isFinite(Number(r.value)))return alert('目前公式無法產生可用數值：'+(r.error||'請檢查公式'));const now=new Date().toISOString();if(editingIndicatorId){const old=customIndicators.find(x=>x.id===editingIndicatorId);if(!old){editingIndicatorId=null;return alert('找不到要編輯的指標，請重新選擇')}const duplicate=customIndicators.find(x=>x.id!==old.id&&x.name===name);if(duplicate)return alert('已有同名指標，請使用其他名稱');old.name=name;old.formula=formula;old.updatedAt=now;editingIndicatorId=null;$('#saveIndicator').textContent='儲存新指標'}else{const old=customIndicators.find(x=>x.name===name);if(old){old.formula=formula;old.updatedAt=now}else customIndicators.push({id:'IND_'+Date.now().toString(36),name,formula,createdAt:now})}saveIndicators();renderCustomIndicators();renderIndicatorAlertControls();renderIndicatorDashboard()};
...
function renderIndicatorDashboard(){
  ...
  if(grid){
    ...
    $('[data-dashboard-edit]').forEach(b=>b.onclick=()=>{const ind=customIndicators.find(x=>x.id===b.dataset.dashboardEdit);if(!ind)return;editingIndicatorId=ind.id;if($('#spreadFormula'))$('#spreadFormula').value=ind.formula;if($('#indicatorName'))$('#indicatorName').value=ind.name;const save=$('#saveIndicator');if(save)save.textContent='更新指標';calcFormula();setTimeout(()=>$('#indicatorEditorPanel')?.scrollIntoView({behavior:'smooth',block:'start'}),80)});
  }
  if(alertsEl){
...
function contractExpiry(id,y,m){if(/^TAIFEX_(TX|MTX|SPF|UNF|UDF|SXF|F1F)$/.test(id)||/^(CME_MES|CME_MNQ|CBOT_MYM|CME_SOX|ICE_Z)$/.test(id))return nthWeekday(y,m,5,3);if(id==='TAIFEX_TJF')return prevBizDay(nthWeekday(y,m,5,2));if(['JPX_TOPIX','JPX_MINI_TOPIX'].includes(id))return nthWeekday(y,m,5,2);if(/^TAIFEX_(RHF|XEF|XJF|XBF|XAF)$/.test(id))return secondBizBeforeThirdWed(y,m);if(/^CME_(CNH|6E|6J|6B|6A)$/.test(id))return nthWeekday(y,m,3,3);return new Date(y,m,0,23,59,59)}
...
function futuresCompareItem(id){return futuresCompareUniverse().find(x=>x.id===id)||null}
function futuresCompareMonths(item){
  const actual=[...(item?.q?.contracts||[])].filter(c=>c?.month).map(c=>c.month);
  return actual.length?[...new Set(actual)].sort():contractMonths(item?.id);
}
function futuresCompareQuote(item,month){
  if(!item)return null;
  const c=(item.q?.contracts||[]).find(x=>String(x.month)===String(month));
...
function fillFuturesCompareMonths(sel,item,preferred){
  if(!sel||!item)return;
  const months=futuresCompareMonths(item);
  sel.innerHTML=months.map(m=>'<option value="'+m+'">'+monthLabel(m)+'</option>').join('');
  sel.value=months.includes(preferred)?preferred:(months[0]||'');
}
function futuresCompareNotional(item,month){
  const q=futuresCompareQuote(item,month),p=Number(q?.last)||((Number(q?.bid)+Number(q?.ask))/2);
  if(Number.isFinite(p)&&p>0)return p*Number(item.spec.multiplier||1)*fxToTwd(item.spec.currency);
  return Number(item.spec.multiplier||1)*fxToTwd(item.spec.currency);
}
function suggestedFuturesRatio(A,B,am,bm){
...
function renderFuturesSpecCompare(resetMonths=false){
  ...
  fillFuturesCompareMonths(am,A,oldAm);fillFuturesCompareMonths(bm,B,oldBm);
  const aq=futuresCompareQuote(A,am?.value),bq=futuresCompareQuote(B,bm?.value);
  const tickValue=x=>Number(x.spec.multiplier)*Number(x.spec.tick);
  const marginTwd=x=>x.margin.initial==null?null:Number(x.margin.initial)*fxToTwd(x.margin.currency);
  const rows=[
    ['交易所',x=>x.exchange],['商品代碼',x=>x.code],['合約乘數',x=>tidy(x.spec.multiplier,6)+' '+x.spec.currency],['最小跳動 Tick',x=>fixedInput(x.spec.tick,8)],['每 Tick 價值',x=>tidy(tickValue(x),6)+' '+x.spec.currency],['幣別',x=>x.spec.currency],['原始保證金',x=>marginDisplay(x.margin)],['保證金約當 TWD',x=>{const v=marginTwd(x);return v==null?'動態':tidy(v,0)+' TWD'}],['到期週期',x=>x.spec.cycle],['規格說明',x=>x.spec.note||'—']
  ];
  box.innerHTML=`<table class="futures-spec-table"><thead><tr><th>規格</th><th>${A.name}<small>${A.exchange} · ${A.code}</small></th><th>${B.name}<small>${B.exchange} · ${B.code}</small></th></tr></thead><tbody>${rows.map(([n,fn])=>`<tr><th>${n}</th><td>${fn(A)}</td><td>${fn(B)}</td></tr>`).join('')}</tbody></table>`;
  const updated=$('#futuresCompareUpdated');if(updated)updated.textContent='更新日期｜A '+futuresCompareDate(aq)+' ｜ B '+futuresCompareDate(bq);
  updateFuturesRatioHint();
  calcFuturesIntegrated();
}
...
function applySuggestedFuturesRatio(){
  ...
  if($('#futuresCompareAQty'))$('#futuresCompareAQty').value=r.a;
  if($('#futuresCompareBQty'))$('#futuresCompareBQty').value=r.b;
  calcFuturesIntegrated();
}
...
function initCostLab(){if(!DATA||!$('#futuresCompareA'))return;bindFuturesSpecCompare();costInitialized=true}
...
function exportCixBackup(){
  ...
  link.href=url;link.download='global-market-monitor-my-index-'+new Date().toISOString().slice(0,10)+'.json';
  document.body.appendChild(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);
}
async function importCixBackup(file){
  try{
    const raw=JSON.parse(await file.text()),rows=Array.isArray(raw)?raw:raw?.customIndexLibrary;
    if(!Array.isArray(rows)||!rows.length)throw new Error('備份中沒有可還原的我的指數');
    const clean=rows.filter(x=>x&&typeof x.name==='string'&&typeof x.symbol==='string'&&typeof x.formula==='string');
    if(!clean.length)throw new Error('備份格式不正確');
    customIndexLibrary=clean;saveCixLibrary();ensureJpyCixPresets();renderCixLibrary();alert('已還原 '+clean.length+' 個我的指數');
  }catch(err){alert('還原失敗：'+(err?.message||err))}
}
...
const CIX_SIGNAL_KEY='gmmCixSignalHistoryV1';
function cixSignalHistory(){try{return JSON.parse(localStorage.getItem(CIX_SIGNAL_KEY)||'[]')}catch{return []}}
function saveCixSignalHistory(v){localStorage.setItem(CIX_SIGNAL_KEY,JSON.stringify(v.slice(0,100)))}
...
function jpySignalHistoryHtml(x){
...
function cixTokenDetail(token){
  ...
  const m=token.match(/^([A-Z][A-Z0-9_]*)(?:\.(BID|ASK|LAST))?$/);if(!m)return null;
  const [,id,field='LAST']=m,row=catalogRows().flatMap(x=>[{side:x.tw,label:`${x.name}｜${x.tw.exchange} ${x.tw.code}`},{side:x.os,label:`${x.name}｜${x.os.exchange} ${x.os.code}`}]).find(x=>x.side?.id===id);
  if(!row)return null;
...
function switchView(name){$('.view').forEach(v=>v.classList.toggle('active',v.id===name+'View'));$('[data-view]').forEach(x=>x.classList.toggle('active',x.dataset.view===name));if(name==='connections')renderConnections();if(name==='indicators'){renderIndicatorDashboard();renderCixLibrary()}if(name==='customindex'){renderCixTokenOptions();renderCixTemplateOptions();updateCixFormulaPreview()}window.scrollTo({top:0,behavior:'smooth'})}
...
async function refresh(){const status=$('#feedStatus');try{if(status)status.textContent='更新中';let raw;try{raw=await MarketProviders.load()}catch(providerError){console.warn('Provider load failed; falling back to core JSON',providerError);const r=await fetch('./data/latest.json?t='+Date.now(),{cache:'no-store'});if(!r.ok)throw new Error('Core JSON HTTP '+r.status);raw=await r.json()}DATA=(window.MarketProviders?.simulate?MarketProviders.simulate(raw):raw);if(status)status.textContent=DATA?.taifexMeta?.generatedAt?'TAIFEX 官方資料已更新':'核心行情已載入';render()}catch(e){console.error('Market refresh failed',e);if(status)status.textContent='資料讀取失敗：'+String(e?.message||e)}}
// Bind primary navigation first so optional module errors can never disable page switching.
$$('[data-view]').forEach(x=>x.onclick=()=>switchView(x.dataset.view));
try{bindStaticUI()}catch(e){console.error('optional UI init failed',e)}
// Re-bind after optional UI setup in case an earlier initializer failed midway.
$$('[data-view]').forEach(x=>x.onclick=()=>switchView(x.dataset.view));
refresh();
