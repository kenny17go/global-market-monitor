...
function symbolMap(){const commodities=Array.isArray(DATA?.commodities)?DATA.commodities:[],equities=Array.isArray(DATA?.equities)?DATA.equities:[],ust=Array.isArray(DATA?.ust)?DATA.ust:[],m={GOLD:commodities[0]?.spot??null,SILVER:commodities[1]?.spot??null,WTI:commodities[2]?.spot??null,BRENT:commodities[3]?.spot??null,USDTWD:DATA?.twd?.spotBid??null,USDTWD_1M:DATA?.twd?.ndf?.p1m??null,USDTWD_3M:DATA?.twd?.ndf?.p3m??null,SPX:equities[0]?.cash??null,SOX:equities[2]?.cash??null,TAIEX:equities[3]?.cash??null,NIKKEI:equities[4]?.cash??null,TOPIX:equities[5]?.cash??null};ust.forEach(x=>m['US'+x.tenor]=x.yield);Object.assign(m,quoteTokenMap());return m}
