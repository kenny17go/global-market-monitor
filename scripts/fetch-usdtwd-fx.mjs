...
async function yahooUsdtwdSeries(){try{
  const j=JSON.parse(await get('https://query1.finance.yahoo.com/v8/finance/chart/USDTWD%3DX?interval=5m&range=1d'));
  const r=j?.chart?.result?.[0],m=r?.meta;
  if(!r||!m)throw new Error('Yahoo USD/TWD chart missing');
  ...
  for(let i=0;i<Math.min(ts.length,close.length);i++){
    const t=Number(ts[i]),v=num(close[i]);
    if(!Number.isFinite(t)||v==null||v<20||v>50)continue;
    series.push(v);seriesTimes.push(new Date(t*1000).toISOString());
  }
  if(series.length<2)throw new Error('Yahoo USD/TWD intraday series too short');
  const last=series.at(-1),previousClose=num(m.chartPreviousClose??m.previousClose);
  const change=previousClose!=null?last-previousClose:null,pct=previousClose?change/previousClose*100:null;
  return {series,seriesTimes,seriesMeta:{range:'1D',interval:'5m',session:'LATEST_SESSION',source:'Yahoo Finance (USDTWD=X)',mode:'DELAYED',timezone:m.exchangeTimezoneName||null,points:series.length},previousClose,change,pct,seriesLast:last,seriesTimestamp:seriesTimes.at(-1)};
}catch(e){console.warn('Yahoo USD/TWD intraday failed:',e.message);return {series:[],seriesTimes:[],seriesMeta:{range:'1D',interval:'5m',session:'LATEST_SESSION',source:'Yahoo Finance (USDTWD=X)',mode:'UNAVAILABLE',points:0},previousClose:null,change:null,pct:null,seriesLast:null,seriesTimestamp:null}}}
...
const spot=await netdaniaSpot();const intraday=await yahooUsdtwdSeries();Object.assign(spot,intraday);let offshore=await netdaniaOffshore();if(!Object.keys(offshore.curve||{}).length)offshore=await barchartOffshore();let onshore=await investingOnshore();if(!Object.keys(onshore.curve||{}).length)onshore=await cbondsOnshore();if(!Object.keys(onshore.curve||{}).length)onshore=await botOnshore();const validSpot=spotOk(spot.bid,spot.ask);
...
const out={meta:{generatedAt:new Date().toISOString(),realtime:false,spotValidated:validSpot,sourcePolicy:'Offshore = NetDania; fallback whole curve to Barchart. Onshore = Investing.com; fallback whole curve to Cbonds; final fallback to Bank of Taiwan exact-tenor forwards minus same-bank spot.',note:'Spot quote uses NetDania; real 1D/5m USD/TWD trend uses Yahoo Finance USDTWD=X. BOT parser anchors on (USD) regardless of page language, then uses exact 30D/90D/180D forwards for 1M/3M/6M.'},spot,onshore,offshore,spread};
