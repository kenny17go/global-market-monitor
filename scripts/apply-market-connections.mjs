...
if(!index.includes('data-view="connections">行情連線')){
  index=index.replace('<span data-view="spread">價差比較</span>','<span data-view="spread">價差比較</span><span data-view="connections">行情連線</span>');
}
...
const connectionsView=`
      <section id="connectionsView" class="view">
        ...
        <article class="panel connections-panel">
          <div class="connections-head"><div><h3>國內券商</h3><p>主要用於 TAIFEX 即時行情。連線需透過使用者自己的本機 Bridge 或私人後端。</p></div></div>
          <div class="broker-grid" id="domesticBrokerGrid"></div>
        </article>
        <article class="panel connections-panel">
          <div class="connections-head"><div><h3>國外券商 / 行情服務</h3><p>可用於 CME、JPX / OSE、ICE 等海外期貨；實際可用市場取決於帳戶行情訂閱。</p></div></div>
          <div class="broker-grid" id="overseasBrokerGrid"></div>
        </article>
        <article class="panel gateway-panel">
          ...
          <div class="gateway-form">
            <label>目前選擇<select id="brokerSelect"></select></label>
            <label>Gateway URL<input id="brokerEndpoint" placeholder="例：http://127.0.0.1:8787/quotes 或 https://your-private-gateway.example/quotes"></label>
            <label>備註<input id="brokerNote" placeholder="例如：家用 Mac mini / Cloudflare Worker"></label>
          </div>
...
// ---- Provider reads non-sensitive connection preference from LocalStorage ----
...
console.log('Applied Market Data Connections UI and gateway preference logic.');
