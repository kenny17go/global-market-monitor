...
index=index.replace(/\s*<div id="taifexSettlementNdf"[\s\S]*?<\/div>\s*<div class="settlement-ndf-note">[\s\S]*?<\/div>\s*<\/div>/g,'');
...
if(!app.includes('function renderTaifexSettlementNdf('))app=app.replace('function renderUsdtwdFx(){',js+'\nfunction renderUsdtwdFx(){');
app=app.replace(/function renderUsdtwdFx\(\)\{syncUsdtwdVisibility\(\);/,'function renderUsdtwdFx(){syncUsdtwdVisibility();setTimeout(renderTaifexSettlementNdf,0);');
